...
const CohortCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            course: _entity?.course?._id,name: _entity?.name,start: _entity?.start,end: _entity?.end,format: _entity?.format,capacity: _entity?.capacity,enrolledCount: _entity?.enrolledCount,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Cohort updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Cohort" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Cohort" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="cohort-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="end"  value={_entity?.end ? new Date(_entity?.end) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("end", new Date(e.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["end"]) ? (
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="capacity" className="w-full mb-3 p-inputtext-sm" value={_entity?.capacity} onChange={(e) => setValByKey("capacity", e.value)}  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="enrolledCount" className="w-full mb-3 p-inputtext-sm" value={_entity?.enrolledCount} onChange={(e) => setValByKey("enrolledCount", e.value)}  />
            </span>
            <small className="p-error">
