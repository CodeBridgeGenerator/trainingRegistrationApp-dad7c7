...
import courseCategoriesFakerFactory from "./courseCategoriesFakerFactory";
...
const CourseCategoriesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = courseCategoriesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("courseCategories").create(elem));
...
export default connect(mapState, mapDispatch)(CourseCategoriesFakerDialogComponent);
