...
const CoursesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            category: _entity?.category?._id,title: _entity?.title,description: _entity?.description,
    ...
    useEffect(() => {
                    ...
                    client
                        .service("courseCategory")
                        .find({ query: { $limit: 10000, $sort: { createdAt: -1 }, _id : urlParams.singleCourseCategoryId } })
                        .then((res) => {
                            setCategory(res.data.map((e) => { return { name: e['name'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.debug({ error });
                            props.alert({ title: "CourseCategory", type: "error", message: error.message || "Failed get courseCategory" });
                        });
                }, []);
    ...
    return (
        <Dialog header="Create Courses" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
