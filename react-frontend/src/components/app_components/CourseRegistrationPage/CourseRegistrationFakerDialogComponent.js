...
import courseRegistrationFakerFactory from "./courseRegistrationFakerFactory";
...
const CourseRegistrationFakerDialogComponent = (props) => {
...
const [profileIds, setprofileIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("cohort")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setcohortIds(data.data);
                        });
client.service("profiles")
    ...
    const onRun = async () => {
        let fakeData = courseRegistrationFakerFactory(props.user,fakerCount,cohortIds,profileIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("courseRegistration").create(elem));
...
export default connect(mapState, mapDispatch)(CourseRegistrationFakerDialogComponent);
