...
import cohortFakerFactory from "./cohortFakerFactory";
...
const CohortFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cohortFakerFactory(props.user,fakerCount,courseIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cohort").create(elem));
...
export default connect(mapState, mapDispatch)(CohortFakerDialogComponent);
