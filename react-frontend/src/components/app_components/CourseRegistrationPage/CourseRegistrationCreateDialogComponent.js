...
const CourseRegistrationCreateDialogComponent = (props) => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const eagerResult = await client.service("courseRegistration").find({
        query: {
          $limit: 10000,
          _id: { $in: [result._id] },
          $populate: [
      ...
      props.alert({
        type: "success",
        title: "Create info",
        message: "Info Registration updated successfully",
      });
      ...
      props.alert({
        type: "error",
        title: "Create",
        message: "Failed to create in Registration",
      });
    }
    setLoading(false);
  };
  ...
  const cohortOptions = cohort.map((elem) => ({
    name: elem.name,
    value: elem.value,
  }));
  ...
  return (
    ...
    >
      <div
        ...
        style={{ maxWidth: "55vw" }}
        role="courseRegistration-create-dialog-component"
      >
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <Dropdown
              id="cohort"
              value={_entity?.cohort?._id}
              optionLabel="name"
              optionValue="value"
              options={cohortOptions}
              onChange={(e) => setValByKey("cohort", { _id: e.value })}
            />
          </span>
        ...
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <Dropdown
              id="payment"
              value={_entity?.payment}
              options={paymentOptions}
              optionLabel="name"
              optionValue="value"
              onChange={(e) => setValByKey("payment", e.value)}
            />
          </span>
          <small className="p-error">
